...
// Client-side state types for easier management
interface Option {
  id: string | number;
  text: string;
  isCorrect: boolean;
...
export default function MultiChoiceCreator({ lesson }: MultiChoiceCreatorProps) {
    ...
    if (isEditMode && lesson) {
      ...
      if (
        ...
        const existingQuestions = lesson.multiChoiceQuestions.map(
          (q, qIndex) => ({
            ...q,
            id: qIndex + 1, // Use temporary numeric IDs for client state
    ...
    setQuestions([
      ...questions,
      {
        ...
        options: [
          ...
          { id: nextOptionId + 1, text: '', isCorrect: false },
        ],
      },
    ]);
      ...
      questions.map((q) =>
        q.id === questionId
          ? {
              ...
              options: q.options.map((o) => ({
                ...o,
                isCorrect: o.id.toString() === correctOptionId,
              })),
            }
          : q
      )
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    // ✨ REFINEMENT: Clearer and more robust validation logic.
    if (!title.trim()) {
    ...
    const validQuestions = questions
      ...
      .map((q) => ({
        // Strip client-side IDs before sending to API
        question: q.question,
    ...
    if (validQuestions.length === 0) {
      setError(
        "You must create at least one valid question with text, at least two options with text, and a selected correct answer."
      );
      setIsLoading(false);
      return;
    }
          ...
          return (
            <div key={q.id} className="rounded-lg border bg-gray-50 p-4">
              <div className="mb-4 flex items-center justify-between">
                <Label htmlFor={`question-${q.id}`} className="text-lg font-semibold">
                  Question {index + 1}
                </Label>
