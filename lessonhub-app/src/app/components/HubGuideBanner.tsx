...
export default function HubGuideBanner({ guideCount = 0, className, copy }: HubGuideBannerProps) {
  ...
  return (
    <div
      className={cn(
        'relative overflow-hidden rounded-3xl border border-border bg-gradient-to-r from-slate-50 via-white to-slate-50 dark:from-slate-950 dark:via-slate-900 dark:to-slate-950 p-6 text-foreground shadow-[0_20px_60px_rgba(0,0,0,0.45)] backdrop-blur',
        className
      )}
    >
      ...
      <button
        ...
        className="absolute right-3 top-3 inline-flex h-8 w-8 items-center justify-center rounded-full border border-border bg-background/70 text-muted-foreground transition hover:border-teal-500/50 hover:text-foreground"
      >
        <X className="h-4 w-4" />
      </button>
      <div className="flex flex-col gap-4 md:flex-row md:items-center md:justify-between">
        <div className="space-y-2 md:max-w-3xl">
          <p className="text-xs font-semibold uppercase tracking-wide text-teal-600 dark:text-teal-200">
            {copy?.bannerKicker || 'Hub Guides'}
          </p>
          <h2 className="text-3xl font-bold text-foreground">{copy?.bannerTitle || 'Always-on practice hub'}</h2>
        ...
        <div className="flex flex-col items-start gap-2 md:items-end">
          ...
          <div className="text-sm font-medium text-teal-600/80 dark:text-teal-100/80">{guideCountLabel}</div>
        </div>
      </div>
    </div>
  );
