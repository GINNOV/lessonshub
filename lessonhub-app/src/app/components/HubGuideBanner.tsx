...
export default function HubGuideBanner({ guideCount = 0, className, copy, banners }: HubGuideBannerProps) {
  ...
  useEffect(() => {
    ...
    const id = setInterval(() => {
      setActiveIndex((prev) => {
        const next = prev + 1;
        return next >= (banners?.length ?? 1) ? 0 : next;
      });
    }, 7000);
    return () => clearInterval(id);
  }, [hasCarousel, banners?.length]);

  if (!visible) {
    return null;
  }

  const activeBanner = hasCarousel ? banners![activeIndex] : null;
      ...
      <div className="flex flex-col gap-4 md:flex-row md:items-center md:justify-between">
        <div className="space-y-2 md:max-w-3xl">
          <p className="text-xs font-semibold uppercase tracking-wide text-teal-200">
            {activeBanner?.bannerKicker || copy?.bannerKicker || 'Hub Guides'}
          </p>
          <h2 className="text-3xl font-bold text-slate-50">{activeBanner?.bannerTitle || copy?.bannerTitle || 'Always-on practice hub'}</h2>
        ...
        <div className="flex flex-col items-start gap-2 md:items-end">
          ...
          >
            <Link href={activeBanner?.bannerHref || "/profile?tab=status"}>{activeBanner?.bannerCta || copy?.bannerCta || 'Unlock Hub Guides'}</Link>
          </Button>
          ...
          {banners!.map((_, index) => (
            <button
              ...
              className={cn(
                ...
                activeIndex === index ? "bg-teal-300" : "bg-slate-600"
              )}
